...
class Cake extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data["title"] = 'billing_months';
        $this->data["description"] = 'billing_months';
        // $this->data["view"] = ADMIN_DIR . "cake/index";


        $controller_name = 'billing_months';
        $table_name = "billing_months";
        $table_name_singular = "billing_month";
        ...
        $colums = array();
        $primary_key = '';
        $ignore = array('status', 'order', 'created_by', 'updated_by', 'created_at', 'updated_at');
        $count = 1;
...
$type = 'text';
if (stristr($colum_name, "email")) {
$type = 'email';
}
if (stristr($colum_name, "no") or strstr($colum_name, "tax") or strstr($colum_name, "pay") or strstr($colum_name,
"deduction")) {
$type = 'number';
}
if (stristr($colum_name, "date")) {
$type = 'date';
}
if (stristr($colum_name, "gender")) {
$type = 'radio';
}
if ($type == 'radio') {
$add_form .= '<div class="form-group row">
    <label for="' . $colum_name . '" class="col-sm-4 col-form-label">' . ucwords(strtolower(str_replace('_', ' ',
        $colum_name))) . '</label>
    <div class="col-sm-8">
        ...
        <input <?php echo $checked ?> type="' . $type . '" required id="<?php echo $option; ?>"
            name="' . $colum_name . '" value="<?php echo $option; ?>" class="">
...
$add_form .= '<div class="form-group row">
    <label for="' . $colum_name . '" class="col-sm-4 col-form-label">' . ucwords(strtolower(str_replace('_', ' ',
        $colum_name))) . '</label>
    <div class="col-sm-8">
        <input type="' . $type . '" required id="' . $colum_name . '" name="' . $colum_name . '"
            value="<?php echo $input->' . $colum_name . '; ?>" class="form-control">
    </div>
</div>
';
}
    ...
    <table class="table table-bordered" id="' . $controller_name . '">
        ...
        <tbody>
            <?php
                ...
                foreach ($rows as $row) { ?>
            <tr>
                <td><a href="<?php echo site_url(ADMIN_DIR . \'' . $controller_name . '/delete_' . $table_name . '/\' . $row->' . $primary_key . '); ?>"
                ...
                $list_view .= '<td><button
                        onclick="get_' . $table_name_singular . '_form(\'<?php echo $row->' . $primary_key . '; ?>\')">Edit
                        <botton>
                </td>';
                $list_view .= '
            </tr>
...
<script>
function get_ ' . $table_name_singular . '
_form(' . $primary_key . ') {
        $.ajax({
                method: "POST",
    ...
    $("#datatable").DataTable({
...

            {
                ...
                "render": function(data, type, row) {
                    return\ '<a class="llink llink-view" href="<?php echo site_url(ADMIN_DIR . "' . $controller_name . '/trash/"); ?>\' + row.'
                        .$primary_key.
                    ...
                    href =
                        "<?php echo site_url(ADMIN_DIR . "' . $controller_name . '/view_' . $table_name . '/"); ?>\' + row.'
                        .$primary_key.
                    ' + \'/\' + \'"><i class="fa fa-eye"></i></a><span style="margin-left: 10px;"></span>\' +\
                    ' < a class = "llink llink-edit"
                    href =
                        "<?php echo site_url(ADMIN_DIR . "' . $controller_name . '/edit/"); ?>\' + row.'
                        .$primary_key.
...
$controller .= '$this->form_validation->set_rules("' . $colum_name . '", "' . ucwords(strtolower(str_replace('_', ' ',
$colum_name))) . '", "required");
';
}
$controller .= '
